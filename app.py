...
def normalize_kindergarten_number(text: str) -> str:
    ...
    if match:
        ...
        if suffix:
            # Нормализуем СП/СП в "СП"
            suffix_upper = suffix.upper()
            return f"{number} {suffix_upper}"
        else:
            return number
    
    # Если не найдена числовая часть, вернуть оригинальный текст
    return text.strip()

def validate_kindergarten_number(text: str) -> Tuple[bool, str]:
    """
    Валидация и нормализация номера детского сада.
    Возвращает: (is_valid, normalized_number_or_error_message)
    """
    if not text or not text.strip():
        return False, '''❌ Пожалуйста, напишите номер детского сада
...
→ Попробуйте еще раз:'''
    ...
    if not any(c.isdigit() for c in normalized):
        return False, '''❌ Пожалуйста, напишите номер детского сада (должны быть цифры)
...
def get_registration_step_4() -> str:
    """Четвёртый вопрос - ФИО родителя"""
    return '''✨ Спасибо за информацию!

🔹 ВОПРОС 4️⃣ из 7️⃣

Ваше фамилия, имя и отчество (опекуна)
...
def get_registration_step_5() -> str:
    """Пятый вопрос - Телефон родителя"""
    return '''📞 Спасибо! Нужен контактный номер...
...
def get_registration_step_6(age: int = 0) -> str:
    """Шестой вопрос - Выбор программы"""
...
{get_registration_step_2()}'''
        ...
        # Переходим к вопросу 3 (номер детского сада)
...
→ Ваш ответ:''', False
        ...
        try:
            ...
            kg_id_search = int(base_number)
            kg = get_kindergarten_info(kg_id_search)
        except:
            kg = None
...
→ Попробуйте еще раз:'''
        ...
        return get_registration_step_6(age) if age > 0 else get_registration_step_6(), True
    
    elif step == 6:  # Выбор программы
        msg = message_text.lower().strip()
        program = PROGRAMS.get(msg)
        if not program:
            return '''❌ Программа не найдена. Выберите из списка:

🤖 robo_34 | 🧱 brick | ⚙️ pro | 🏆 pro_plus | 💃 dance | 🗣️ logoped | 📚 school_2 | ✏️ school_1
...
→ Напишите код программы:''', False
    ...
    elif step == 7:  # Подтверждение
        msg = message_text.lower().strip()
        if msg in ['да', 'да!', 'подтверждаю', 'подтверждаю!', 'верно', 'верно!']:
            ...
            return '', True
        elif msg in ['нет', 'нет!', 'исправить', 'изменить']:
